...
var States;
(function (States) {
    States["NOTASSIGNEDGAME"] = "NOT ASSIGNED GAME";
    States["INGAMEWAITING"] = "IN GAME WAITING";
    States["INGAMEPLAYING"] = "IN GAME PLAYING";
    States["GAMEENDED"] = "GAME ENDED";
})(States || (States = {}));
...
function isClientScrolledDown() {
    return (Math.abs($("#inner")[0].scrollTop +
        $("#inner")[0].clientHeight -
        $("#inner")[0].scrollHeight) <= 10);
}
...
$(function () {
    ...
    if ($("#lobbyItemList .lobbyItem").length == 0) {
        ReactDOM.render(React.createElement("p", { id: "emptyLobbyItemPrompt", style: {
                textAlign: "center",
                marginTop: "20px",
                fontStyle: "italic",
                fontSize: "1.1em",
            } }, "Create a new game to play"), $("#lobbyItemList")[0]);
    }
    $("#registerBox").focus();
    leaveGameButton.setNotInPlayClick();
    ...
    user.socket.on("getAllRolesForSelection", function (rolesArray) {
        ...
        for (let roles of rolesArray) {
            $("#allRolesForGameType").append('<button class="ui ' +
                roles.color +
                ' button">' +
                roles.name +
                "</button>");
    ...
    user.socket.on("updateGame", function (name, playerNames, playerColors, number, inPlay) {
        ...
        for (let i = 0; i < playerNames.length; i++) {
            if (i == 0) {
                div.append('<span class="username" style="color:' +
                    playerColors[i] +
                    '">' +
                    playerNames[i]);
            }
